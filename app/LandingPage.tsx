...
const services = [
  { title: 'Bordados', description: 'Logotipos, gorras, uniformes y piezas con terminación premium.', icon: Scissors, tone: 'pink' },
  { title: 'DTF / Sublimación', description: 'Impresión de alta calidad para textiles, marcas y eventos.', icon: Palette, tone: 'blue' },
  { title: 'Uniformes', description: 'Opciones empresariales, escolares, deportivas y promocionales.', icon: Shirt, tone: 'yellow' },
  { title: 'Láser', description: 'Grabados en madera, acrílico, metal y artículos personalizados.', icon: Zap, tone: 'dark' },
  { title: 'Promocionales', description: 'Mugs, termos, llaveros, lapiceros y regalos corporativos.', icon: Gift, tone: 'pink' },
  { title: 'Personalización', description: 'Creamos piezas únicas a partir de tu idea, marca o evento.', icon: WandSparkles, tone: 'blue' },
];

const work = [
  { title: 'Bordado corporativo', category: 'Bordados', image: 'https://images.unsplash.com/photo-1523398002811-999ca8dec234?auto=format&fit=crop&w=900&q=80' },
  { title: 'Mugs personalizados', category: 'Sublimación', image: 'https://images.unsplash.com/photo-1514228742587-6b1558fcca3d?auto=format&fit=crop&w=900&q=80' },
  { title: 'Textiles con identidad', category: 'DTF', image: 'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?auto=format&fit=crop&w=900&q=80' },
  { title: 'Regalos para marcas', category: 'Promocionales', image: 'https://images.unsplash.com/photo-1549465220-1a8b9238cd48?auto=format&fit=crop&w=900&q=80' },
  { title: 'Uniformes profesionales', category: 'Textil', image: 'https://images.unsplash.com/photo-1523381294911-8d3cead13475?auto=format&fit=crop&w=900&q=80' },
  { title: 'Detalles hechos a medida', category: 'Personalización', image: 'https://images.unsplash.com/photo-1455390582262-044cdead277a?auto=format&fit=crop&w=900&q=80' },
];
...
const testimonials = [
  { name: 'Yanelis Campusano', text: 'Excelente calidad y servicio. Mis uniformes quedaron espectaculares. Súper recomendados.' },
  { name: 'Cliente empresarial', text: 'Muy profesionales, atentos y con trabajos de gran calidad. El resultado superó lo esperado.' },
  { name: 'Cliente satisfecha', text: 'Me encantaron mis artículos personalizados. Buen precio, excelente atención y entrega impecable.' },
];
    ...
    <main>
      <header className="site-header">
        <div className="container nav-wrap">
          ...
          <button className="menu-button" onClick={() => setMenuOpen((value) => !value)} aria-label="Abrir menú" aria-expanded={menuOpen}>
            {menuOpen ? <X /> : <Menu />}
          </button>
        </div>

        {menuOpen && (
          <motion.nav initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="mobile-nav">
      ...
      <section id="inicio" className="hero">
        ...
        <div className="container hero-grid">
          <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="hero-copy">
            ...
            <p>
              Damos vida a tus ideas con bordados, impresión, láser y artículos personalizados. Calidad, creatividad y compromiso en cada detalle.
            </p>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.12 }} className="hero-showcase">
            ...
            <div className="product-float mug-card"><span className="product-emoji">☕</span><b>Tazas</b><small>Personalizadas</small></div>
            <div className="product-float cap-card"><span className="product-emoji">🧢</span><b>Gorras</b><small>Bordadas</small></div>
            <div className="product-float laser-card"><span className="product-emoji">🔑</span><b>Láser</b><small>Grabado premium</small></div>
        ...
        <div className="container">
          ...
          <div className="services-grid">
            {services.map((service, index) => {
              ...
              return (
                <motion.article key={service.title} className={`service-card tone-${service.tone}`} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ delay: index * 0.06 }} whileHover={{ y: -8 }}>
        ...
        <div className="container">
          <div className="portfolio-top">
            ...
            <div className="portfolio-grid">
              {work.map((item, index) => (
                <motion.a key={item.title} href="https://www.instagram.com/zafygraf/" target="_blank" rel="noreferrer" className={`work-card ${index === 0 ? 'work-tall' : ''}`} initial={{ opacity: 0, scale: 0.97 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} whileHover={{ scale: 1.015 }}>
        ...
        <div className="container catalog-layout">
          ...
          <div className="catalog-grid">
            {catalog.map(([title, emoji], index) => (
              <motion.a href={whatsappUrl} target="_blank" rel="noreferrer" className="catalog-card" key={title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.04 }} whileHover={{ y: -7, rotate: index % 2 === 0 ? -1 : 1 }}>
                <span>{emoji}</span>
                <b>{title}</b>
              </motion.a>
            ))}
        ...
        <div className="container">
          ...
          <div className="testimonials-grid">
            {testimonials.map((testimonial, index) => (
              <motion.article key={testimonial.name} className="testimonial-card" initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08 }}>
